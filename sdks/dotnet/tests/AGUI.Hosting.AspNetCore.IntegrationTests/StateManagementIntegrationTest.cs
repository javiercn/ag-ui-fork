...
public sealed class StateManagementIntegrationTest : IntegrationTestBase
{
    ...
    [Fact]
    public async Task PostRun_StateSnapshot_MapsToUpdateWithRawRepresentation()
    {
        var client = CreateClient((messages, options, ct) => EmitStateSnapshotResponse(ct));
    ...
    [Fact]
    public async Task PostRun_StateDelta_MapsToUpdateWithRawRepresentation()
    {
        var client = CreateClient((messages, options, ct) => EmitStateWithDeltaResponse(ct));
    ...
    [Fact]
    public async Task PostRun_StateEvents_ShareConversationAndResponseIds()
    {
        var client = CreateClient((messages, options, ct) => EmitStateWithDeltaResponse(ct));
    ...
    [Fact]
    public async Task PostRun_WithStateInRawRepresentationFactory_StateArrivesOnServer()
    {
        RunAgentInput? capturedInput = null;
        var client = CreateClient((messages, options, ct) =>
        {
            capturedInput = options?.AdditionalProperties?["agui_input"] as RunAgentInput;
    ...
    [Fact]
    public async Task PostRun_WithNullState_ServerReceivesNullState()
    {
        RunAgentInput? capturedInput = null;
        var client = CreateClient((messages, options, ct) =>
        {
    ...
    [Fact]
    public async Task PostRun_WithStateRoundTrip_ServerReceivesAndEchoesState()
    {
        var client = CreateClient((messages, options, ct) =>
        {
            var input = options?.AdditionalProperties?["agui_input"] as RunAgentInput;

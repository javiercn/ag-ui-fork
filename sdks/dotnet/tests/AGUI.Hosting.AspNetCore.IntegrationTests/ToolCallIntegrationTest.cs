...
public sealed class ToolCallIntegrationTest : IntegrationTestBase
{
    ...
    [Fact]
    public async Task PostRun_SequentialToolCalls_EmitsAllEventsInOrder()
    {
        var client = CreateClient((messages, options, ct) =>
            EmitSequentialToolCalls(ct));
    ...
    [Fact]
    public async Task PostRun_ToolCallInterleavedWithText_BothEmitted()
    {
        var client = CreateClient((messages, options, ct) =>
            EmitToolCallWithText(ct));
    ...
    [Fact]
    public async Task PostRun_ToolCallWithNullArgs_MapsToFunctionCallContentWithNullArgs()
    {
        var client = CreateClient((messages, options, ct) =>
            EmitToolCallResponse("call-1", "ping", null, ct));

...
public sealed class ClientToolIntegrationTest : IntegrationTestBase
{
    ...
    [Fact]
    public async Task ClientTools_SentInRunAgentInput_ConvertedToAITools()
    {
    ...
    [Fact]
    public async Task ClientToolAutoInvocation_ServerRequestsClientTool_FunctionInvokedByFunctionInvokingChatClient()
    {
    ...
    [Fact]
    public async Task ServerToolPassThrough_NotInClientTools_YieldedAsFunctionCallContent()
    {
        ...
        var client = CreateClient((messages, options, ct) =>
            EmitToolCallResponse("call-1", "server_only_tool",
                new Dictionary<string, object?> { ["key"] = "value" }, ct));
    ...
    [Fact]
    public async Task ServerToolPassThrough_WithClientTools_ServerToolNotAutoInvoked()
    {
        var clientTool = AIFunctionFactory.Create(
            (string x) => x,
            "client_tool",
        ...
        var client = CreateClient((messages, options, ct) =>
            EmitToolCallResponse("call-1", "server_tool",
                new Dictionary<string, object?> { ["data"] = "test" }, ct));

...
public sealed class TextStreamingIntegrationTest : IntegrationTestBase
{
    ...
    public async Task PostRun_TextContent_MapsToTextMessageEvents()
    {
        var client = CreateClient((messages, options, ct) =>
            EmitTextResponse("Hello world!", ct));

        var updates = await CollectUpdates(client, [new ChatMessage(ChatRole.User, "Hi")]);

        // All updates should share the same ConversationId and ResponseId
    ...
    [Fact]
    public async Task PostRun_Streaming_YieldsTextContent()
    {
        var client = CreateClient((messages, options, ct) =>
            EmitTextResponse("Hello via ChatClient!", ct));
    ...
    [Fact]
    public async Task PostRun_MultiTurn_ViaGetResponseAsync()
    {
        var turns = new Queue<Func<IEnumerable<ChatMessage>, ChatOptions?, CancellationToken, IAsyncEnumerable<ChatResponseUpdate>>>();
    ...
    [Fact]
    public async Task PostRun_AuthorName_RoundTrips()
    {
        var client = CreateClient((messages, options, ct) =>
            EmitTextResponseWithAuthorName("Hello!", "TestAgent", ct));

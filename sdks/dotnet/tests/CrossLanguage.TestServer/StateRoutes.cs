...
internal static class StateRoutes
{
        ...
        string pattern)
    {
        ...
        {
            return TypedResults.ServerSentEvents(EmitSharedStateAsync(input, jsonOptions.Value.SerializerOptions, cancellationToken));
        });
    }
        ...
        string pattern)
    {
        return endpoints.MapPost(pattern, (
            ...
            CancellationToken cancellationToken) =>
        {
            return TypedResults.ServerSentEvents(EmitPredictiveStateAsync(input, cancellationToken));
        });
    }

    private static async IAsyncEnumerable<SseItem<BaseEvent>> EmitSharedStateAsync(
        RunAgentInput input,
        ...
        [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        yield return new SseItem<BaseEvent>(new StateSnapshotEvent
        {
            Snapshot = snapshot,
        });
    ...
    private static async IAsyncEnumerable<SseItem<BaseEvent>> EmitPredictiveStateAsync(
        RunAgentInput input,
        [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken)
    {
        yield return new SseItem<BaseEvent>(new RunStartedEvent
        ...
        for (int i = 0; i < paragraphs.Length; i++)
        {
            ...
            yield return new SseItem<BaseEvent>(new StateDeltaEvent
            {

...
The standard constructor creates the built-in HTTP transport internally:
...
<Note>
  ...
  `HttpClient` constructor. Most applications do not need to instantiate a
  transport directly.
</Note>
...
IChatClient client = new AGUIChatClient(transport);
```

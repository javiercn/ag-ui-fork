...
Create a client directly from an `HttpClient` and the AG-UI endpoint URL:
...
IChatClient client = new AGUIChatClient(httpClient, "https://api.example.com/agent");
```

The public constructors are:

```csharp
new AGUIChatClient(HttpClient httpClient, string endpoint, JsonSerializerOptions? jsonSerializerOptions = null);
new AGUIChatClient(IAGUITransport transport);
new AGUIChatClient(IAGUITransport transport, JsonSerializerOptions jsonSerializerOptions);
```

Use the `IAGUITransport` overloads when you need a custom transport for tests or
for an alternative wire protocol. See [Transport](/sdk/dotnet/client/transport).
